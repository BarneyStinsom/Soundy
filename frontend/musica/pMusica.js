// pMusica.js — player de uma música: capa, barra de progresso, anterior / play-pause / próxima.
// Também registra a reprodução no histórico depois de 30 s ouvidos (POST /playhistory).
// Depende de ../js/api.js (API_URL, userId, api, formatDuration)
...
function showCover(url) {
    ...
    if (url) {
        cover.src = new URL(url, API_URL).href;
    }
}
...
async function init() {
    if (!startId) {
        message.textContent = 'Endereço inválido: falta o id da música. Use pMusica.html?id=...';
        return;
    }
