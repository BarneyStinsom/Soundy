...
// Quem decide é o parâmetro "tipo" da URL:  pgMusicas.html?tipo=playlist&id=...
// Se o "tipo" não vier (só ?id=...), a página descobre sozinha se o id é de álbum ou de playlist.
// Cada música tem um link "Tocar" que abre o player (musica/pMusica.html) dentro desta coleção.
...
let currentSongs = []; // [{ rowId, position, id, title, duration }]
let allSongs = [];     // todas as músicas do sistema (pra adicionar na playlist)

// ---------- CARREGAR CONFORME O TIPO ----------

// Cada loader devolve { title, tipoLabel, songs } no mesmo formato.
...
async function loadAlbum() {
    ...
    return {
        ...
        tipoLabel: album.type, // "album", "ep", "single"...
...
// ---------- PLAYLIST: ADICIONAR E REMOVER MÚSICAS ----------
